...
In this post I'll explore the K Means algorithm to cluster [data](../data/spotify_songs.csv) fetched from spotify API. This data containes the following attributes
...
sns.pairplot(songs)
```
This plots can say a lot about the data, e.g., songs in the lower end of danceability spectrum are less likely to be popular. Aside from that, most of the features seem to have a fairly normal distrubtion on the data.
## Normalization
Let's scales data to a similar range to esnure consistancey in impact of the features.
